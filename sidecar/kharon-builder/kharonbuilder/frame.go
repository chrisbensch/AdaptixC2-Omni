...
// *KharonBuildRequest, hands it to handler, then writes either the *KharonBuildResponse
// or a *KharonBuildError frame, and closes. A handler error is surfaced on the wire
// as a *KharonBuildError and ServeConn returns nil — the exchange completed, only the
// outcome was an error. Only transport-level failures (bad frame, write error)
// are returned to the caller.
func ServeConn(conn net.Conn, handler func(*KharonBuildRequest) (*KharonBuildResponse, error)) error {
	defer conn.Close()

	var req KharonBuildRequest
	if err := ReadFrame(conn, &req, MaxFrameBytes); err != nil {
		// Best effort: tell the peer what went wrong, then abort.
		_ = WriteFrame(conn, &KharonBuildError{Error: err.Error()})
		return err
	}

	resp, err := handler(&req)
	if err != nil {
		if werr := WriteFrame(conn, &KharonBuildError{Error: err.Error()}); werr != nil {
			return werr
		}
