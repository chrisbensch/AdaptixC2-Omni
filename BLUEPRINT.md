...
10. **Runtime privilege reduction:** the server runs as an unprivileged `adaptix` user (UID/GID 10001) inside the container. Read-only rootfs is **on by default** now that both payload builders compile off-server — the `kharon-builder` sidecar compiles the Kharon beacon/loader and the `nax-builder` compiles the Nax payload, so the server writes nothing to the rootfs. Every server write target (SQLite DB, listener metadata, downloads, uploads, screenshots, cert/key, profile, credentials.txt) lives under `/app/data` (the server never touches `/tmp`, so no tmpfs is needed). `ADAPTIX_READ_ONLY=false` is only for debugging a stray rootfs write. `cap_drop:[ALL]` + `cap_add:[CHOWN, SETUID, SETGID, NET_BIND_SERVICE]`, and `security_opt:no-new-privileges`. The entrypoint stays root only long enough to chown the bind mount + render the profile, then `exec gosu`s to UID 10001. `setcap cap_net_bind_service=+ep` on the server binary preserves the one cap operator listeners need (`:80`/`:443`/`:53`) across the gosu UID drop — every other cap stays dropped. CI smoke-tests the same posture (§12.1).
...
services:
  ...
  server:                                      # profile: runtime — runs the server
    ...
    network_mode: host
    read_only: ${ADAPTIX_READ_ONLY:-true}      # on by default (both payload builders compile off-server in their sidecars); flip false to debug a stray rootfs write
    cap_drop: [ALL]
    cap_add:
...
`read_only: true` is feasible now that **both** payload builders compile off-server (the `kharon-builder` sidecar compiles the Kharon beacon/loader, the `nax-builder` compiles the Nax payload), so the server itself writes nothing to the rootfs. Every server write target (SQLite DB, listener metadata, downloads, uploads, screenshots, cert/key, profile, credentials.txt) lives under `/app/data` — verified against `AdaptixC2/AdaptixServer/core/utils/logs/repo.go` (`DataPath`/`DbPath`/`ListenerPath`/`DownloadPath`/`UploadPath`/`ScreenshotPath` all derive from `os.Getwd() + "/data"`, and the CMD launches with cwd `/app`). **The server never touches `/tmp` (no `os.TempDir`/tempfile calls in the core or agent), so no tmpfs is required.** `no-new-privileges:true` prevents any setuid binaries from re-escalating after the gosu drop.
...
### 5.10 `kharon-builder` sidecar

The Kharon PIC beacon + loader compile **off-server** in the `kharon-builder` sidecar, so the runtime teamserver image ships no compiler and no writable Kharon source tree. This is what makes `read_only: true` feasible for a Kharon deployment (see §10, §5.2).

**`Dockerfile.kharon-builder`** — multi-stage (mirrors `Dockerfile.nax-builder`):

- `base` (golang + mingw-w64 + clang/nasm/llvm + binutils) → `prebuild-x64` / `prebuild-x86` (the Kharon `src_beacon` Makefile prebuild, run for both arches so the runtime image carries no toolchain).
- `builder` (debian-slim) — the runtime image. Ships `curl` (HEALTHCHECK), `gosu`, `libcap2-bin`, and the unprivileged `kharonb` user/group **UID/GID 10003** (distinct from `adaptix` 10001 and `naxb` 10002). The pinned Kharon source tree is copied to `/app/kharon` (read-only at runtime).

**Compose wiring** (`docker-compose.yml`, `kharon-builder` service + `kharon-sock` volume):

- The builder runs as `kharonb` (UID 10003), `network_mode: none`, `cap_drop: [ALL]` + `DAC_OVERRIDE, CHOWN, SETUID, SETGID`, `no-new-privileges:true`.
- It writes its unix socket to `kharon-sock:/run/kharon` (a named volume). The `server` service mounts the same `kharon-sock:/run/kharon` and its `agent_kharon` plugin dials `/run/kharon/builder.sock` — exactly like the NaX path (`nax-sock:/run/nax`). The server only **connects** the socket; the builder owns the volume.
- `kharonb` (10003) is the owning group so the builder writes the socket and the server (joined to `kharonb` via the Dockerfile `usermod -a -G kharonb adaptix`) can read it — no DAC on the server.

**Patch application (`build-server` stage).** The `patches/adaptix-kharon-sidecar.patch` is applied with `patch -p2` from `AdaptixServer/extenders/agent_kharon/` (headers are `a/agent_kharon/…`). Order matters: the makefile patch (`all: clean plugin agent` → `all: clean plugin`, which drops the in-server `make agent` beacon build) is applied **before** the sidecar patch — both then match under strict `git apply`, not just GNU patch's fuzzy context. After the patch, `../sidecar/kharon-builder` is added to the `go work use` line so `go work sync` resolves the `kharonbuilder` import the patched plugin carries.

**arm64 objcopy fix (`patches/kharon-beacon-objcopy.patch`).** On arm64 the host default `objcopy` is an ARM triple that cannot read x86-64 PE files, so the PIC beacon `.bin` comes out 0 bytes unless the patch repoints objcopy at `x86_64-w64-mingw32-objcopy`. This is proven green in CI on both arches (§12 step 5); a 0-byte beacon fails the `bin` smoke check.

**CI smoke test (§12 step 5).** Builds the `kharon-builder` image per arch, copies the pinned Kharon tree **out of the freshly built image** (so it tests exactly what the committed image ships), runs the builder sharing a writable `kharon-sock:/run/kharon` + `/app/kharon`, waits for `/run/kharon/builder.sock`, then builds and runs the pure-Go `sidecar/kharon-builder/cmd/smoke` client inside a `golang` container sharing both dirs. It drives a real PIC beacon (`bin`) build then a loader (`exe`) build through the socket — proving socket comms + the mingw/objcopy fix produce non-empty payloads on THIS arch.
...
4. **Smoke test** — `docker run -d` the freshly-built image with `-p 4321:4321`, a writable `./data` mount, `ADAPTIX_TEAMSERVER_PASSWORD=ci-smoke-pw`, and the same hardened posture compose enforces in §5.2 (`--cap-drop ALL` + the four cap-adds, `--security-opt no-new-privileges:true`). read_only stays off here (a writable rootfs tolerates stray startup writes; the server no longer compiles payloads — both builders compile off-server in their sidecars, so `ADAPTIX_READ_ONLY=true` is valid and is what the hardened runtime posture in §5.2 enforces); Then poll `docker inspect --format '{{.State.Health.Status}}'` every 3 seconds up to 30 iterations (~90s budget) waiting for the HEALTHCHECK to report `healthy`. Once healthy, assert PID 1 in the container is running as UID 10001 (`adaptix`) by reading `/proc/1/status` via `docker exec` — catches a dropped `exec gosu` in the entrypoint — and that `/app/404page.html` + the rendered `/app/data/profile.yaml` don't carry framework-identifying strings (no `AdaptixC2` in the 404 body, no `Server: "AdaptixC2"` or `Adaptix-Version:` headers in the profile). Catches a missed Dockerfile COPY override or a profile-template revert. On failure, dumps `docker logs` for diagnosis.
5. **Build + smoke-test the Nax builder sidecar** — `docker build -f Dockerfile.nax-builder -t adaptixc2-omni-nax-builder:${{ matrix.runner }} .` (per arch), then drive a real build through the builder's unix socket with the existing `sidecar/nax-builder/cmd/smoke` client. The step copies the pinned NaX tree **out of the freshly built image** (so it tests exactly what the committed image ships, not host working-tree state), runs the builder with a shared writable `/run/nax`, waits for the socket, builds the smoke client inside a `golang` container, and runs it sharing both `/run/nax` (socket) and `/nax` (source tree). The smoke client reads `Config.h` + `Config_sleepmask.h` from the pinned tree, synthesizes the one header it lacks (`Config_profile.h`, value-trivial `0x00` so the beacon *compiles*), and builds — proving socket comms + `writeConfigHeaders` + the mingw/objcopy fix produce a non-empty payload on THIS arch. Runs as root (build-correctness probe; privilege model is covered by the runtime smoke test). A broken objcopy triple — the host default `objcopy` on arm64, which can't read x86-64 PE files — fails the build with a 0-byte component, so CI catches that regression. `timeout-minutes: 20`.
6. **Vulnerability scan (Trivy)** — `aquasecurity/trivy-action@v0.36.0` scans `adaptixc2-omni:latest` for OS + library CVEs. Fails on `CRITICAL` or `HIGH` severity with `ignore-unfixed: true` — we only block on things we can act on (bump base-image digest, bump apt package). Runs after smoke so we don't scan a broken image.
7. **SBOM generation** — `trivy-action` again, this time with `format: cyclonedx` and `output: sbom-${{ matrix.runner }}.cyclonedx.json`.
8. **Upload SBOM** — `actions/upload-artifact@v4` attaches the per-arch SBOM to the workflow run (`sbom-ubuntu-latest` / `sbom-ubuntu-24.04-arm`, 30-day retention). Not consumed by anything in CI today; available for audit/compliance use.
9. **Teardown** — `docker rm -f` the smoke container in an `if: always()` step so the runner is clean for the next job.

Total wall time: about **7–8 minutes** for both arches together (the two matrix jobs run in parallel; the build dominates each at ~7 min and the smoke test is sub-90s once the image is up). The Trivy scan + SBOM add ~30–60 seconds per arch on top — most of that is the one-time CVE-database download. An earlier draft of this section quoted 12–18 minutes per arch — that was a conservative estimate from the first runs and has been borne out as too high by ~6 PRs of measured runs. The nax-builder build + smoke (§12 step 5) adds bounded time on top of this — each arch re-runs a `make components` prebuild plus a socket round-trip, but both new steps carry a `timeout-minutes` cap so a hang can't consume the 45-minute job budget.
...
- **A real NaX profile decode.** The nax-builder smoke synthesizes a value-trivial (`0x00`) `Config_profile.h` so the beacon *compiles* — it does not decode into a live C2 callback. A real-profile run needs the server's pure-Go generators + a running teamserver to drive through (see Milestone 2 status doc).
