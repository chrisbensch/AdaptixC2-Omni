...
10. **Runtime privilege reduction:** the server runs as an unprivileged `adaptix` user (UID/GID 10001) inside the container. Read-only rootfs is **off by default** — the Kharon agent compiles beacon/loader at runtime, which writes into the rootfs. A **Nax-only** deployment can enable it (`read_only: true` via `ADAPTIX_READ_ONLY=true`) because Nax compiles in the nax-builder sidecar and every server write target lives under `/app/data` (the server never touches `/tmp`, so no tmpfs is needed). `cap_drop:[ALL]` + `cap_add:[CHOWN, SETUID, SETGID, NET_BIND_SERVICE]`, and `security_opt:no-new-privileges`. The entrypoint stays root only long enough to chown the bind mount + render the profile, then `exec gosu`s to UID 10001. `setcap cap_net_bind_service=+ep` on the server binary preserves the one cap operator listeners need (`:80`/`:443`/`:53`) across the gosu UID drop — every other cap stays dropped. CI smoke-tests the same posture (§12.1).
...
services:
  ...
  server:                                      # profile: runtime — runs the server
    ...
    network_mode: host
    read_only: ${ADAPTIX_READ_ONLY:-false}     # off by default (Kharon compiles at runtime); a Nax-only deployment can flip it on
    cap_drop: [ALL]
    cap_add:
...
`read_only: true` is feasible **for a Nax-only deployment** because every server write target (SQLite DB, listener metadata, downloads, uploads, screenshots, cert/key, profile, credentials.txt) lives under `/app/data` — verified against `AdaptixC2/AdaptixServer/core/utils/logs/repo.go` (`DataPath`/`DbPath`/`ListenerPath`/`DownloadPath`/`UploadPath`/`ScreenshotPath` all derive from `os.Getwd() + "/data"`, and the CMD launches with cwd `/app`). **The server never touches `/tmp` (no `os.TempDir`/tempfile calls in the core or Nax agent), so no tmpfs is required.** `no-new-privileges:true` prevents any setuid binaries from re-escalating after the gosu drop.
