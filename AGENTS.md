...
| `NaX/` | Submodule. [MaorSabag/NaX](https://github.com/MaorSabag/NaX) — a second agent ecosystem integrated as additional server extenders (`agent_nonameax`, `listener_nonameax_http`, `listener_nonameax_smb`, `service_nax_store`). Its C++ beacon is **not** compiled in the server image; it's built on request by the `nax-builder` sidecar over a Unix socket (see below). Research/planning record: [`NAX-INTEGRATION.md`](./NAX-INTEGRATION.md). |
...
| File | Purpose |
|---|---|
| `Dockerfile` | Multi-stage build, **host-arch by default** (verified arm64 + amd64): `base` (Go 1.25.12 + mingw-w64 + clang/nasm/llvm; `go-win7` pinned via `ARG GO_WIN7_SHA`) → `build-bofs` (Extension-Kit with the nanodump patch applied — 9 BOFs strict, SAL-BOF best-effort because of its network fetch — + PostEx-Arsenal) → `build-server` (AdaptixServer dependency patch + server + 9 extenders, including Kharon grafted at build time) → `runtime` (debian-slim with `curl` for HEALTHCHECK, `gosu` for privilege drop, and `libcap2-bin` for `setcap`; unprivileged `adaptix` user UID/GID 10001 created; `setcap cap_net_bind_service=+ep` on `/app/adaptixserver` so listeners can bind `:80/:443/:53` after the UID drop clears caps; entrypoint COPYed from `docker/entrypoint.sh`; `EXPOSE 4321` only; `HEALTHCHECK` probes `https://127.0.0.1:4321/endpoint`). Force a specific arch with `DOCKER_DEFAULT_PLATFORM=linux/amd64`. |
| `docker-compose.yml` | Three profiles: `build` (host-arch), `runtime` (host-networking server with `./data` bind-mount only — no profile bind-mount; profile is rendered into `data/profile.yaml` on first start; **runs with `read_only: true`, `cap_drop:[ALL]` + `cap_add:[CHOWN, SETUID, SETGID, NET_BIND_SERVICE]`, `security_opt:no-new-privileges`, `mem_limit: 4g` / `pids_limit: 4096` (overridable via `ADAPTIX_MEM_LIMIT` / `ADAPTIX_PIDS_LIMIT`), and a 30 MB bounded JSON log** so the server runs as UID 10001 with a minimal cap set and can't take down the host — note there is **no `tmpfs:/tmp`** (the server never writes there), plus the two named socket volumes `nax-sock:/run/nax` and `kharon-sock:/run/kharon` the builder sidecars write into), `nax-builder` + `kharon-builder` (the two payload-builder sidecars; see below), `build-client` (Linux AppImage — multi-arch via `ADAPTIX_CLIENT_*` env vars, defaults to `linux/amd64` for backward compatibility). Optional `ADAPTIX_TEAMSERVER_PASSWORD` / `ADAPTIX_OPERATORS` env vars pin credentials at first render. |
...
| `scripts/build-client-linux.sh` | Wrapper around the `build-client` compose profile. `cd`s into `$REPO_ROOT`, exports the invoking numeric UID/GID so the copy container creates host-owned artifacts, and applies `patches/adaptixclient-linux-appimage.patch` with automatic reversal. It drives amd64 → patched `build-client` (Ubuntu 22.04 + Qt 6.9.2 + linuxdeployqt) or arm64 → `build-client-kali` (Kali rolling + distro Qt 6.10.2 + linuxdeploy). Both paths use checksum-locked modern appimagetool assets plus an explicit dated type-2 runtime. `--arch host\|amd64\|arm64` defaults to host; host != target runs under QEMU; `--clean` wipes `AdaptixClient-dist/` while preserving the layer cache. |
...
| `scripts/setup-local.sh` | Local-only dev helper: adds the two layout-specific `go.work use` entries (pinned NaX agent module + `sidecar/`) so `go vet`/`go test`/`go build` resolve the agent→sidecar local `replace` on the host. The Docker build strips and re-adds container-correct paths, so this is **not** committed inside the submodule. Idempotent, callable from any cwd. |
| `patches/` | Build-time patches against submodule trees. We don't own the upstream repos, so persistent customizations live here as unified diffs and are applied/reverted by the relevant build step. Currently: `adaptixserver-go-dependencies.patch` (raises fixed Go module floors inside `build-server`), `adaptixclient-macos-bundle.patch` (macOS bundle CMake block, host-applied with auto-revert), `adaptixclient-linux-appimage.patch` (modern checksum-locked amd64/arm64 AppImage packaging plus the Kali arm64 stage, host-applied with auto-revert), and `extension-kit-nanodump-host-strip.patch` (nanodump host-arch strip fix, applied inside the build container). |
...
| `profile.nax-only.yaml` | Example Nax-only server profile (loads ONLY `agent_nonameax` + the Nax listeners + `service_nax_store`; no beacon/gopher/Kharon, no AxScripts). Because Nax payloads compile in the `nax-builder` sidecar, the server does no runtime compilation and never writes to the rootfs — this is the deployment shape that runs with `read_only: true`. Copy to `./data/profile.yaml` before first start to use it. |
| `NAX-INTEGRATION.md` | Research + decision record for integrating the NaX agent ecosystem (compatibility, build separation, read-only-rootfs feasibility). Not a live integration doc; the actual integration is described in `README.md` / `BLUEPRINT.md` / `AGENTS.md` below. |
| `Dockerfile.nax-builder` | Image for the `nax-builder` sidecar: pinned `NaX/` source + cross toolchain, runs as `naxb` (UID 10002), writes its builder Unix socket to `/run/nax`. |
| `Dockerfile.kharon-builder` | Image for the `kharon-builder` sidecar: pinned `Kharon/` source + cross toolchain, runs as `kharonb` (UID 10003), writes its builder Unix socket to `/run/kharon`. |
...
## Nax + Kharon payload-builder sidecars

Payload compilation runs **off the teamserver**, in two builder sidecars reached over a per-`Build` Unix socket. This is what lets the hardened runtime run `read_only: true`: the server image ships no compiler and no writable source tree, and every runtime write goes to `./data`.

- `agent_kharon` dials `kharon-builder`'s socket at `/run/kharon/builder.sock`; `agent_nonameax` (NaX) dials `nax-builder`'s at `/run/nax/builder.sock`. The server **only connects** — the builder owns the make/build and writes the `.o`/`.bin`.
- The server image (Dockerfile `build-server`/`runtime` stages) applies `patches/adaptix-kharon-sidecar.patch` and `patches/adaptix-nax-sidecar-task6.patch` (both `patch -p2`), copies `sidecar/` into the workspace, and `go work use`s both builders so the plugins' local `replace` directives resolve.
- `nax-builder` runs as user `naxb` (UID 10002), `kharon-builder` as `kharonb` (UID 10003) — distinct from the server's `adaptix` (UID 10001). The server is `usermod`-joined to both groups so it can read the builders' `0o660` sockets on the shared named volumes without `DAC_OVERRIDE`.
- The two builder sockets live in named volumes (`nax-sock:/run/nax`, `kharon-sock:/run/kharon`) — a named volume, **not** tmpfs, so the socket is shared between the server and its builder container.
- Per-arch builder images: `docker compose --profile runtime build nax-builder kharon-builder` (or `docker build -f Dockerfile.nax-builder` / `Dockerfile.kharon-builder`).
